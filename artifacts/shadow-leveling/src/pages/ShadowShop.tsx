...
export default function ShadowShop() {
  ...
  const onPurchase = (id: string, cost: number, name: string) => {
    if ((character?.gold || 0) < cost) {
      toast({
        title: "Insufficient Gold",
        description: `You need ${cost.toLocaleString()} G to redeem ${name}.`,
        variant: "destructive",
      });
      return;
    }
    ...
    purchaseItem.mutate(
      { id },
      {
        onSuccess: (res) => {
          ...
          toast({
            title: `Redeemed: ${res.itemName}`,
            description: `Spent ${res.goldSpent.toLocaleString()} G — Remaining: ${res.goldRemaining.toLocaleString()} G`,
    ...
    return (
      <div className="p-8 flex flex-col items-center justify-center h-full gap-4">
        ...
        <p className="text-muted-foreground tracking-widest uppercase text-sm">
          System Error — Shadow Shop Offline
        </p>
    ...
    <div className="p-6 md:p-8 max-w-6xl mx-auto">
      ...
      {items.length === 0 ? (
        <div className="text-center py-20 border border-dashed border-white/10 rounded-xl glass-panel">
          <p className="text-muted-foreground tracking-widest">SHADOW SHOP INVENTORY EMPTY</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item) => {
            ...
            return (
              <Card
                key={item.id}
                className={`glass-panel overflow-hidden transition-all duration-300 relative ${
                  canAfford
                    ? "hover:border-primary/50 hover:shadow-[0_0_24px_rgba(124,58,237,0.18)]"
